...
function explainExpoPushError(err: unknown): string {
  ...
  if (/timed out|TIMEOUT/i.test(raw)) {
    return (
      'Expo push token timed out. Check internet, update Google Play Services, ' +
      'sign into a Google account, set automatic date & time, then tap Test again. ' +
      'Avoid Force stop — it can block push until the next open.'
    );
  }
  ...
  if (/SERVICE_MISSING|SERVICE_DISABLED|SERVICE_INVALID/i.test(raw)) {
    return (
      'Google Play Services is missing or disabled on this phone — push cannot work here.'
    );
  }
